...
# ==============================================================================
# 0. AUTOMATIC BARE-MODE & BROWSER OPENER
# ==============================================================================
def open_browser():
...
# ==============================================================================
# 1. CORE MACHINE LEARNING PIPELINE (Real Kaggle Datasets Ingestion)
# ==============================================================================
...
# File paths for your Kaggle/Custom CSV files
SALES_CSV = "sales_transactions.csv"
CALENDAR_CSV = "calendar_dates.csv"
ITEMS_CSV = "pharmacy_products.csv"


def generate_fallback_kaggle_datasets():
    """Generates realistic Kaggle-structured datasets if files are missing."""
    if not (os.path.exists(SALES_CSV) and os.path.exists(CALENDAR_CSV) and os.path.exists(ITEMS_CSV)):
        ...
        pd.DataFrame(cal_list, columns=['Date', 'Season', 'IsHoliday', 'IsPromotional']).to_csv(CALENDAR_CSV,
                                                                                                index=False)
        ...
        pd.DataFrame(products, columns=['ItemID', 'ProductGroup', 'Category', 'BasePrice']).to_csv(ITEMS_CSV,
                                                                                                   index=False)


def run_ml_pipeline():
    """Ingests Kaggle datasets, transforms features, and trains the model."""
    generate_fallback_kaggle_datasets()
...
st.title("📊 Meditrak Multi-Store Demand Forecasting Platform")
st.caption("Supply Chain Intelligent Inventory Framework Powered by Kaggle Datasets")
st.markdown("---")
...
with tab1:
    ...
    with c2:
        target_month = st.slider("Target Projection Month", 1, 12, 7)
...
with tab2:
    ...
    st.write("Automated replenishment recommendations based on real historical patterns:")
...
with tab3:
    ...
    with g1:
        ...
        sns.lineplot(data=monthly_trend, x='Month', y='UnitsSold', marker='o', color='teal', linewidth=2.5, ax=ax)
    ...
    with g2:
        ...
        fig, ax = plt.subplots()
        sns.boxplot(data=df_merged, x='StoreID', y='UnitsSold', palette="Set2", ax=ax)
        st.pyplot(fig)
